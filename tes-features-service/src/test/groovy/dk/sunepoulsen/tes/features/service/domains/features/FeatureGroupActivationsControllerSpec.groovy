...
class FeatureGroupActivationsControllerSpec extends Specification {
    ...
    void "Test get specific activation for feature group: Successfully"() {
        given:
            FeatureActivation foundActivation = new FeatureActivation(id: 17L, enabled: true)

        when:
            DeferredResult<FeatureActivation> deferredResult = sut.getActivation('key', 17)
            DeferredResults.wait(deferredResult)
        ...
        then:
            ...
            1 * featureGroupsLogic.getActivation('key', 17) >> CompletableFuture.completedFuture(foundActivation)
            0 * _
    }

    void "Test get specific activation for feature group: Logic layer returns LogicException"() {
        when:
            DeferredResult<FeatureActivation> deferredResult = sut.getActivation('key', 17)
            DeferredResults.wait(deferredResult)
            throw deferredResult.getResult() as Throwable
        ...
        then:
            ...
            1 * featureGroupsLogic.getActivation('key', 17) >> CompletableFuture.failedFuture(
                new ResourceNotFoundException('code', 'param', 'message')
            )
            0 * _
    }

    void "Test get specific activation for feature group: Logic layer throws LogicException"() {
        when:
            sut.getActivation('key', 17)
        ...
        then:
            ...
            1 * featureGroupsLogic.getActivation('key', 17) >> {
                throw new ResourceNotFoundException('code', 'param', 'message')
            }
            0 * _
    }

    void "Test patch specific activation for feature group: Successfully"() {
        given:
        ...
        when:
            DeferredResult<FeatureActivation> deferredResult = sut.patchActivation('key', 17, newActivation)
            DeferredResults.wait(deferredResult)
        ...
        then:
            ...
            1 * featureGroupsLogic.patchActivation('key', 17, newActivation) >> CompletableFuture.completedFuture(returnedActivation)
            0 * _
    }

    void "Test patch specific activation for feature group: Logic layer returns LogicException"() {
        given:
            FeatureActivation newActivation = new FeatureActivation(enabled: false)
        ...
        then:
            ...
            1 * featureGroupsLogic.patchActivation('key', 17, newActivation) >> CompletableFuture.failedFuture(
                new ResourceNotFoundException('code', 'param', 'message')
            )
            0 * _
    }

    void "Test patch specific activation for feature group: Logic layer throws LogicException"() {
        given:
            FeatureActivation newActivation = new FeatureActivation(enabled: false)

        when:
            sut.patchActivation('key', 17, newActivation)
        ...
        then:
            ...
            1 * featureGroupsLogic.patchActivation('key', 17, newActivation) >> {
                throw new ResourceNotFoundException('code', 'param', 'message')
            }
            0 * _
    }
