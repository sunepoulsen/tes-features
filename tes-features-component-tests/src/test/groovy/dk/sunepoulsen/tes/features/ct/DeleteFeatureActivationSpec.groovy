...
@Slf4j
class DeleteFeatureActivationSpec extends Specification implements FeaturesServiceIntegratorProvider, FeaturesTestsIntegratorProvider, FeaturesMockUsers {
    ...
    void "DELETE /groups/{feature_group_key}/features/{feature_key}/activations/{activation_id} returns OK"() {
        ...
        when: 'DELETE /groups/{feature_group_key}/features/{feature_key}/activations/{activation_id}'
            Void result = featuresServiceIntegrator().features().activations().deleteFeatureActivation(featuresDefaultUser(), registeredFeatureGroup.key, registeredFeature.key, featureActivation.id).blockingAwait()
    ...
    void "DELETE /groups/{feature_group_key}/features/{feature_key}/activations/{activation_id} returns Bad request: #_testcase"() {
        ...
        when: 'DELETE /groups/{feature_group_key}/features/{feature_key}/activations/{activation_id}'
            featuresServiceIntegrator().features().activations().deleteFeatureActivation(featuresDefaultUser(), _featureGroupKey, _featureKey, Long.MAX_VALUE).blockingAwait()
    ...
    void "DELETE /groups/{feature_group_key}/features/{feature_key}/activations/{activation_id} returns not found"() {
        ...
        when: 'DELETE /groups/{feature_group_key}/features/{feature_key}/activations/{activation_id}'
            featuresServiceIntegrator().features().activations().deleteFeatureActivation(featuresDefaultUser(), registeredFeatureGroup.key, registeredFeature.key, Long.MAX_VALUE).blockingAwait()
